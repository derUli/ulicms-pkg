...
function list_files($uid) {
    ...
    if (db_num_rows($files) > 0) {
        ...
        echo "<p class='shared_file_message ulicms-error'>" . "Es sind momentan keine Dateien für diesen Kunden freigegeben." . "</p>";
    }
...
function customer_list() {
    ...
    for ($i = 0; $i < count($users); $i++) {
        $data = getUserByName($users[$i]);
        echo "<li>" . '<a href="index.php?action=module_settings&module=kundenbereich&user=' . $data["id"] . '">' . $data["username"] . "</a></li>";
    }
}

...
class StructureController extends AbstractController {
        ...
        foreach ($this->tables as $keyname => $current_table) {
            ...
            if (!$this->dbIsSystemSchema) {
                ...
                $drop_message = sprintf(
                        ($table_is_view || $current_table['ENGINE'] == null ? __('View %s has been dropped.') : __('Table %s has been dropped.')),
                        str_replace(
                                ' ',
                                '&nbsp;',
                                htmlspecialchars($current_table['TABLE_NAME'])
            ...
            $structure_table_rows[] = [
                ...
                'unit' => $unit,
                'overhead' => $overhead,
                'create_time' => isset($create_time) && $create_time ? Util::localisedDate(strtotime($create_time)) : '-',
                'update_time' => isset($update_time) && $update_time ? Util::localisedDate(strtotime($update_time)) : '-',
                'check_time' => isset($check_time) && $check_time ? Util::localisedDate(strtotime($check_time)) : '-',
                'charset' => $charset ?? '',
                'is_show_stats' => $this->isShowStats,
                'ignored' => $ignored,
                'do' => $do,
    ...
    protected function getReplicationStatus($replicaInfo, string $table): array {
        ...
        if ($replicaInfo['status']) {
            ...
            $do = (is_string($searchDoDBInTruename) && strlen($searchDoDBInTruename) > 0) || (is_string($searchDoDBInDB) && strlen($searchDoDBInDB) > 0) || ($nbServSlaveDoDb == 0 && $nbServSlaveIgnoreDb == 0) || $this->hasTable(
            ...
            $ignored = (is_string($searchTable) && strlen($searchTable) > 0) || (is_string($searchDb) && strlen($searchDb) > 0) || $this->hasTable(
    ...
    protected function hasTable(array $db, $truename) {
        foreach ($db as $db_table) {
            if ($this->db == $this->replication->extractDbOrTable($db_table) && preg_match(
                            '@^' .
    ...
    protected function getStuffForEngineTypeTable(
        ...
        switch ($current_table['ENGINE']) {
            ...
            case 'TokuDB':
                ...
                [$current_table, $formatted_size, $unit, $sum_size] = $this->getValuesForInnodbTable(
                        $current_table,
                        $sum_size
                );
                break;
    ...
    protected function getValuesForAriaTable(
        ...
        if ($this->isShowStats) {
            ...
            if (isset($current_table['Data_free']) && $current_table['Data_free'] > 0
                ...
                [$formatted_overhead, $overhead_unit] = Util::formatByteDown(

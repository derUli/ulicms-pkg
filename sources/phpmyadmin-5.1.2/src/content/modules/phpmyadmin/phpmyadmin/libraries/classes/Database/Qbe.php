...
class Qbe {
    ...
    private function getWhereClause() {
        ...
        for ($row_index = 0; $row_index <= $this->criteriaRowCount; $row_index++) {
            ...
            $where_clause .= "\n"
                    . mb_strtoupper(
                            isset($this->formAndOrRows[$row_index]) ? $this->formAndOrRows[$row_index] . ' ' : ''
                    )
                    . $qry_orwhere;
        }
    ...
    private function getLeftJoinColumnCandidates(
        ...
        [$candidate_columns, $needsort] = $this->getLeftJoinColumnCandidatesBest(
                $search_tables,
                $where_clause_columns,
                $unique_columns,
                $index_columns
    ...
    private function getSavedSearchesField() {
        ...
        foreach ($this->savedSearchList as $id => $name) {
            $html_output .= '<option value="' . htmlspecialchars((string) $id)
                    . '" ' . (
                    $id == $currentSearchId ? 'selected="selected" ' : ''
                    )
                    . '>'

...
class Innodb extends StorageEngine {
    ...
    public function getPageBufferpool() {
        ...
        $output .= '    </tbody>' . "\n"
                ...
                . '            <td class="text-monospace text-right">'
                . ($status['Innodb_buffer_pool_read_requests'] == 0 ? '---' : htmlspecialchars(
                        Util::formatNumber(
                                $status['Innodb_buffer_pool_reads'] * 100 / $status['Innodb_buffer_pool_read_requests'],
                                3,
                                2
                        )
                ) . ' %') . "\n"
                ...
                . '            <td class="text-monospace text-right">'
                . ($status['Innodb_buffer_pool_write_requests'] == 0 ? '---' : htmlspecialchars(
                        Util::formatNumber(
                                $status['Innodb_buffer_pool_wait_free'] * 100 / $status['Innodb_buffer_pool_write_requests'],
                                3,
                                2
                        )
                ) . ' %') . "\n"

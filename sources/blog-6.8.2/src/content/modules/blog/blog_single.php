    ...
    if (db_num_rows($query) > 0) {
        ...
        if ($acl->hasPermission("blog") or $post->entry_enabled) {
            ...
            if ($acl->hasPermission("blog")) {
                $html .= "<a href='" . buildSEOUrl(get_slug()) . "?blog_admin=edit_post&id=" . $post->id . "'>[Bearbeiten]</a> ";

                $html .= "<a href='" . buildSEOUrl(get_slug()) . "?blog_admin=delete_post&id=" . $post->id . "' onclick='return confirm(\"Diesen Post wirklich löschen?\")'>[Löschen]</a>";
            } elseif (logged_in()) {
                $html .= "
		   ...
		   <div class='disabled_link'>[Löschen]</div>";
            ...
            return "<p class='ulicms-error'>Dieser Blogartikel ist momentan deaktiviert.</p>";
        }
    } else {
        return "<p class='ulicms-error'>Dieser Blogartikel existiert nicht mehr.<br/>
...
function blog_display_comments($post_id)
{
    ...
    if (AntiSpamHelper::isCountryBlocked() and $spamfilter_enabled) {
        ...
        if ($_SESSION["language"] == "de") {
           ...
           wenden Sie sich bitte an den Administrator dieser Internetseite.</p>";
            ...
            $html .= "<p class='ulicms-error'>Users from your Country are blocked by the spamfilter. If you believe, this is an error, please contact the administrator.</p>";
        }
    } elseif (getconfig("disallow_chinese_chars") and $spamfilter_enabled and (AntispamHelper::isChinese($_POST["name"]) or AntispamHelper::isChinese($_POST["comment"]))) {
        Settings::set("contact_form_refused_spam_mails", Settings::get("contact_form_refused_spam_mails") + 1);
        if ($_SESSION["language"] == "de") {
            $html .= "<p class='ulicms-error'>" . "Chinesische Schriftzeichen sind nicht erlaubt!</p>";
        } else {
            $html .= "<p class='ulicms-error'>" . "Chinese chars are not allowed!</p>";
        }
    } elseif (getconfig("disallow_cyrillic_chars") and $spamfilter_enabled and (AntispamHelper::isCyrillic($_POST["name"]) or AntispamHelper::isCyrillic($_POST["comment"]))) {
        Settings::set("contact_form_refused_spam_mails", Settings::get("contact_form_refused_spam_mails") + 1);
        if ($_SESSION["language"] == "de") {
            $html .= "<p class='ulicms-error'>" . "Kyrillische Schriftzeichen sind nicht erlaubt!</p>";
        } else {
            $html .= "<p class='ulicms-error'>" . "cyrilic chars are not allowed!</p>";
        }
    } elseif (getconfig("disallow_rtl_chars") and $spamfilter_enabled and (AntispamHelper::isRtl($_POST["name"]) or AntispamHelper::isRtl($_POST["comment"]))) {
        Settings::set("contact_form_refused_spam_mails", Settings::get("contact_form_refused_spam_mails") + 1);
        if ($_SESSION["language"] == "de") {
            $html .= "<p class='ulicms-error'>" .
                    "Sprachen die von rechts nach links gelesen werden, sind nicht erlaubt!</p>";
        } else {
            $html .= "<p class='ulicms-error'>" .
                    "Right to left languages such as hebrew, arabic and persian are not allowed!</p>";
        }
    } elseif ($spamfilter_enabled and (AntiSpamHelper::containsBadwords($_POST["name"])
            or AntiSpamHelper::containsBadwords($_POST["url"])
            or AntiSpamHelper::containsBadwords($_POST["comment"]))) {
        Settings::set("contact_form_refused_spam_mails", Settings::get("contact_form_refused_spam_mails") + 1);
        if ($_SESSION["language"] == "de") {
            $html .= "<p class='ulicms-error'>" . "Ihr Kommentar enthält nicht erlaubte Wörter.</p>";
        } else {
            $html .= "<p class='ulicms-error'>" . "Your comment contains not allowed words.</p>";
        }
    } else {
        $checkbox = new PrivacyCheckbox(getCurrentLanguage(true));
        if (Request::isPost() and $checkbox->isEnabled() and ! $checkbox->isChecked()) {
            $html .= "<p class='ulicms-error'>" . get_translation("please_accept_privacy_conditions") . "</p>";
        } else {
    ...
    if (db_num_rows($query) > 0) {
	   ...
	   <a href='#comment" . $comment->id . "' name='comment" . $comment->id . "'>";
            ...
            if ($acl->hasPermission("blog")) {
                $html .= " <a href='" . buildSEOUrl(get_slug()) . "?blog_admin=delete_comment&id=" . $comment->id . "' onclick='return confirm(\"Diesen Kommentar wirklich löschen?\")'>[Löschen]</a>";
            }

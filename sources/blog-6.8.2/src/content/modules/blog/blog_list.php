...
function blog_list()
{
    ...
    if ($hasPermission) {
        $html .= "<p><a class='add_blog_entry_link' href='" . buildSEOUrl(get_slug()) . "?blog_admin=add'>Blogeintrag anlegen</a></p>";
    }
...
id, author, seo_shortname, title, datum, `views`, language, id, author, seo_shortname, title, datum, `views`, language";
    ...
    if (db_num_rows($query) > 0) {
        while ($post = db_fetch_object($query)) {
            ...
            $html .= "<h2 class='blog_headline'><a href='" . buildSEOUrl(get_slug()) . "?single=" . $post->seo_shortname . "'>" . htmlspecialchars($post->title) . "</a></h2>";
            ...
            if ($_SESSION ["language"] == "de") {
                $html .= "<a href='" . buildSEOUrl(get_slug()) . "?single=" . $post->seo_shortname . "' class='blog-read-more-link'>weiterlesen...</a>
			 ";
            } else {
                $html .= "<br/><a href='" . buildSEOUrl(get_slug()) . "?single=" . $post->seo_shortname . "' class='blog-read-more-link'>read more...</a>
			";
            }
            $html .= "<br/><br/>";
        ...
        if ($limit3 > - 1) {
            $html .= "<a href='" . buildSEOUrl(get_slug()) . "?limit=" . ($limit3) . "'>";
        }
        ...
        if ($last_post_id != $oldest_post_id) {
            $html .= "<a href='" . buildSEOUrl(get_slug()) . "?limit=" . $limit2 . "'>";
        }

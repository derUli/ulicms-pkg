...
function jumbotron_get_menu($name = "top", $parent_id = null, $recursive = true, $order = "position") {
    ...
    $sql = "SELECT id, slug, access, link_url, title, alternate_title, menu_image, target FROM " . tbname("content") . " WHERE menu='$name' AND language = '$language' AND active = 1 AND `deleted_at` IS NULL AND hidden = 0 and parent_id ";

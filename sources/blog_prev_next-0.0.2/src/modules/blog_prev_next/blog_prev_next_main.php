...
function blog_prev_next_render(){
     ...
     $prevQuery = db_query("SELECT title, seo_shortname FROM " . tbname("blog") . " WHERE datum < " . $thisQuery -> datum . " ORDER by datum DESC LIMIT 1");
    
    
     $nextQuery = db_query("SELECT title, seo_shortname FROM " . tbname("blog") . " WHERE datum > " . $thisQuery -> datum . " ORDER by datum ASC LIMIT 1");

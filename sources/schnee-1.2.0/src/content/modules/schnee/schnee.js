...
var snow_colors = new Array('#AAAACC', '#DDDDFF', '#CCCCDD', '#F3F3F3',
        '#F0FFFF');

// die Schriftarten, aus denen die Schneeflocken bestehen sollen
var snow_fonts = new Array('Arial Black', 'Arial Narrow', 'Times',
        'Comic Sans MS');
...
var snow_init_delay = 50; // Verzögerungszeit in Millisekunden, bevor es zu
// schneien anfängt

// ######################################################################
// HIER ENDET DIE KONFIGURATION. ÄNDERUNGEN IM FOLGENDEN SETZEN
// JAVSCRIPT-KENNTNISSE VORAUS
// ######################################################################
...
function createSnowflakes() {
    ...
    for (var i = 0; i <= snow_flake_count; i++)
        document.write('<sp' + 'an id="snwflk' + i + '" style="' + style + '">'
                + snow_char + '</sp' + 'an>');
}
...
function initSnow() {
    ...
    if (!snow_area_el || snow_area_el.offsetWidth <= snow_max_size
            || snow_area_el.offsetHeight <= snow_max_size) {
        // after 5 secs cancel
...
function updateSnow() {
    ...
    for (var i = 0; i <= snow_flake_count; i++) {
        ...
        var x = snowflakes[i].posx
                + (snowflakes[i].wobamp * Math.sin(snowflakes[i].wob));

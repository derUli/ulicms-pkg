...
function PMA_sqlQueryForm($query = true, $display_tab = false, $delimiter = ';')
{
    ...
    if ($is_querywindow){
        ...
        <form method="post" id="sqlqueryform" target="frame_content"
              action="import.php"<?php echo $enctype;
        ?> name="sqlform"
    ...
    if (count($fields_list)){
     ...
     if (! empty($GLOBALS['cfg']['Bookmark'])){
        ...
        <label for="id_bkm_all_users">
            <?php echo __('Let every user access this bookmark');
        ?></label>
        </div>
        <div class="formelement">
        ...
        <label for="id_bkm_replace">
            <?php echo __('Replace existing bookmark of same name');
        ?></label>
        </div>
        </div>
        <?php
     ...
     if ($is_querywindow){
        ...
        //<![CDATA[
            document.writeln(' <input type="checkbox" name="LockFromUpdate" checked="checked" tabindex="120" id="checkbox_lock" /> <label for="checkbox_lock"><?php echo __('Do not overwrite this query from outside the window');
        ?></label> ');
        //]]>
        </script>
        <?php

...
<div id="importmain">
    <img src="<?php echo $GLOBALS['pmaThemeImage'];
...
if ($_SESSION[$SESSION_KEY]["handler"] != "UploadNoplugin"){
     ?>
                ...
                var perform_upload = function () {
                    new $.getJSON(
                    'import_status.php?id=<?php echo $upload_id ;
    ?>&<?php echo PMA_generate_common_url(array('import_status' => 1), '&');
    ?>',
                    {},
                    function(response) {
    ...
    ?>ajax_clock_small.gif" width="16" height="16" alt="ajax clock" /> <?php echo PMA_jsFormat(__('The file being uploaded is probably larger than the maximum allowed size or this is a known bug in webkit based (Safari, Google Chrome, Arora etc.) browsers.'), false);
    ?>');
    ...
    ?>',
                            ...
                            if ($('#importmain').is(':visible')) {
                                ...
                                $('#importmain').hide();
                                $('#import_form_status')
                                    .html('<div class="upload_progress"><div class="upload_progress_bar_outer"><div class="percentage"></div><div id="status" class="upload_progress_bar_inner"><div class="percentage"></div></div></div><div><img src="<?php echo $GLOBALS['pmaThemeImage'];
    ?>ajax_clock_small.gif" width="16" height="16" alt="ajax clock" /> <?php echo PMA_jsFormat(__('Uploading your import file…'), false);
    ?></div><div id="statustext"></div></div>')
                                    .show();
                                import_start = now;
                            }
    ...
    ?>'
                        ...
                        if (finished == true) {
                            ...
                            $('#importmain').hide();
                            $('#import_form_status')
                                .html('<img src="<?php echo $GLOBALS['pmaThemeImage'];
    ?>ajax_clock_small.gif" width="16" height="16" alt="ajax clock" /> <?php echo PMA_jsFormat(__('The file is being processed, please be patient.'), false);
    ?> ')
                                .show();
                            $('#import_form_status').load('import_status.php?message=true&<?php echo PMA_generate_common_url(array('import_status' => 1), '&');
    ...
    <?php
     ...
     ?>
                $('#upload_form_status_info').html('<img src="<?php echo $GLOBALS['pmaThemeImage'];
    ?>ajax_clock_small.gif" width="16" height="16" alt="ajax clock" /> <?php echo PMA_jsFormat(__('Please be patient, the file is being uploaded. Details about the upload are not available.'), false) . PMA_Util :: showDocu('faq', 'faq2-9');
    ?>');
                        $('#upload_form_status').css("display", "none");
    <?php
    } // else
...
?>/>
            <label for="checkbox_allow_interrupt"><?php echo __('Allow the interruption of an import in case the script detects it is close to the PHP timeout limit. <i>(This might be a good way to import large files, however it can break transactions.)</i>');
?></label><br />
        </div>
...
if (! (isset($timeout_passed) && $timeout_passed)){
     ?>
        <div class="formelementrow">
            <label for="text_skip_queries"><?php echo __('Number of rows to skip, starting from the first row:');
    ?></label>
            <input type="text" name="skip_queries" value="<?php echo PMA_pluginGetDefault('Import', 'skip_queries');
    ...
    ?>
        <input type="hidden" name="skip_queries" value="<?php echo PMA_pluginGetDefault('Import', 'skip_queries');
    ?>" id="text_skip_queries" />
            <?php
    }
 ?>
...
?>
    ...
    <div class="importoptions" id="format_specific_opts">
        <h3><?php echo __('Format-Specific Options:');
?></h3>
...
if (function_exists('PMA_set_enc_form')){
    ?>
        <div class="importoptions" id="kanji_encoding">
            <h3><?php echo __('Encoding Conversion:');
    ?></h3>

		...
		while ( $row = db_fetch_object ( $query ) ) {
			if (checkAccess ( $row->access )) {
				...
				if (get_requested_pagename () != $row->slug) {
					$html .= "<a href='" . buildSEOUrl ( $row->slug, $row->redirection ) . "' target='" . $row->target . "' class='" . trim ( $additional_classes ) . "'>";
				} else {
					$html .= "<a class='current-page-item" . rtrim ( $additional_classes ) . "' href='" . buildSEOUrl ( $row->slug, $row->redirection ) . "' target='" . $row->target . "'>";
				}
